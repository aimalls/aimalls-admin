...
import { approveParticipantTaskToAPI, denyParticipantTaskToAPI, getTaskParticipantsFromAPI } from "../../../../../requests/user-task.request";
...
export const ParticipantsList: FC<iProps> = ({taskId}): JSX.Element => {

    const taskParticipantsQuery = useQuery(
        ["taskParticipants"],
        () => getTaskParticipantsFromAPI(taskId)
    )
                            ...
                            <IonGrid>
                                <IonRow>
                                    ...
                                    <IonCol size="12" sizeMd="3" style={{ display: 'flex', justifyContent: 'end' }}>
                                        <IonButton color="danger" onClick={() => denyParticipantTask(participant._id)}>Deny</IonButton>
                                        <IonButton color="primary" onClick={() => approveParticipantTask(participant._id)}>Approve</IonButton>
                                    </IonCol>
                                </IonRow>
                            </IonGrid>
                        </IonCardContent>
